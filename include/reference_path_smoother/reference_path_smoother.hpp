...
class ReferencePathSmoother {
 ...
 private:
    ...
    // search.
    bool graphSearch(std::shared_ptr<PathOptimizationNS::ReferencePath> reference);
    bool graphSearchDp(std::shared_ptr<PathOptimizationNS::ReferencePath> reference);
